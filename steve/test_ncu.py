...
def _set_if_present(obj, field_name: str, value: int) -> bool:
...
def _force_workload_lengths(workload, length: int = 8192):
    updated_fields = []
    candidate_fields = (
        "q_len",
        "kv_len",
    ...
    for field_name in candidate_fields:
        if _set_if_present(workload, field_name, length):
            updated_fields.append(field_name)

    if not updated_fields and hasattr(workload, "model_dump"):
        dumped = workload.model_dump()
        for field_name in candidate_fields:
            if field_name in dumped:
                raise TypeError(
                    "Workload exposes length fields only through model_dump(); "
                    "please update _force_workload_lengths for this workload type."
                )

    return workload, updated_fields


def _workload_metadata(workload):
    if hasattr(workload, "model_dump"):
        return _normalize_json(workload.model_dump())
    if isinstance(workload, dict):
        return _normalize_json(workload)
    return {"repr": str(workload)}
...
def run_ncu_profile(
    ...
    workload, updated_fields = _force_workload_lengths(workload, target_seq_len)
    workload_metadata = _workload_metadata(workload)

    print(f"Selected workload index: {idx}")
    print(f"Forced workload length fields to {target_seq_len}: {updated_fields or 'none'}")
    print("Workload metadata:")
    print(json.dumps(workload_metadata, indent=2, sort_keys=True, default=str))
